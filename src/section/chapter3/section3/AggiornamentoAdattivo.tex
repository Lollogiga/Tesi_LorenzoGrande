...
La soluzione proposta adotta invece un \emph{feedback loop} periodico che
trasforma il profilo energetico in una grandezza adattiva, aggiornata
continuamente in funzione del contesto di esecuzione. Il componente
responsabile di tale meccanismo è un worker di raccolta eseguito in
background, che costituisce il nucleo del controllo retroazionato del
sistema. La Figura~\ref{fig:worker-architecture} illustra il flusso
completo di acquisizione, elaborazione e persistenza dei campioni
energetici.
...
\begin{figure}[ht]
    ...
    di invocazioni dal livello di esecuzione, calcola i delta, applica
    un filtro EMA e aggiorna il profilo persistito in etcd, archiviando
...
Docker attivi e le funzioni in esecuzione. A tal fine è stato introdotto
un registro in memoria, denominato \texttt{ContainerRegistry}, che
associa a ciascun container una struttura \texttt{ContainerState}
contenente i seguenti campi:

\begin{itemize}
    \item l'identificatore del container (\texttt{ContainerID});
    \item il nome della variante (\texttt{FunctionName}), il relativo
    \texttt{LogicalName} e il \texttt{VariantID} che la identifica
    univocamente nella funzione logica;
    \item i valori di baseline dell'energia cumulativa e del numero di
    invocazioni, impiegati per il calcolo dei delta incrementali;
...
La registrazione di un container avviene sia in fase di creazione del
runtime sia, in modo difensivo, al momento dell'invocazione, così da
gestire correttamente il caso di \emph{warm start}, in cui il container
è già attivo ma non ancora registrato. Il processo è tale per cui, se un
container è già presente nel registry, il mapping container-variante
non viene modificato, garantendo coerenza durante l'intero ciclo di vita
del container.

Parallelamente, viene mantenuto un contatore atomico di invocazioni per
container, incrementato al completamento di ogni richiesta. L'impiego
di operazioni atomiche evita l'introduzione di meccanismi di lock nel
\emph{critical path} dello scheduler, preservando le prestazioni anche
in presenza di elevata concorrenza.
...
dell'istanza — e comporta la reinizializzazione della baseline senza
...
Contestualmente all'aggiornamento del profilo in etcd, ogni campione
viene archiviato in InfluxDB come punto della \emph{measurement}
\texttt{energy\_sample}. Ogni punto include tag identificativi del
container e della variante, nonché il campo \texttt{invocation\_joule}
espresso in Joule.

La separazione tra etcd e InfluxDB riflette la doppia funzione che il
dato energetico riveste nel sistema:

\begin{itemize}
    \item etcd supporta il \emph{presente}: fornisce allo scheduler la
    stima corrente con latenza minima, ottimizzando il percorso decisionale
    in fase di selezione della variante;
    \item InfluxDB preserva il \emph{passato}: mantiene la serie storica
    dei campioni energetici, abilitando analisi sperimentali e visualizzazioni grafiche.
\end{itemize}

Questa architettura a doppio livello consente di disaccoppiare le
esigenze operative dello scheduler — latenza e consistenza a runtime —
dalle esigenze analitiche della fase sperimentale.