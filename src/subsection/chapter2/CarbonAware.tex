...
l'impatto ambientale di un'unità di energia consumata non è costante,
ma dipende dalla composizione del mix energetico della rete elettrica
nel momento e nel luogo dell'esecuzione. Un kilowattora consumato quando
la rete è alimentata prevalentemente da fonti rinnovabili --- eolico,
solare, idroelettrico --- produce emissioni di CO$_2$ significativamente
inferiori rispetto allo stesso kilowattora consumato in un momento in
cui la generazione è dominata da combustibili fossili. Questa grandezza,
denominata \emph{carbon intensity} (CI) e tipicamente espressa in
gCO$_2$eq/kWh, varia nel tempo in funzione delle condizioni
meteorologiche, della domanda sulla rete e della disponibilità delle
fonti di generazione, e varia nello spazio in funzione del mix
energetico caratteristico di ciascuna zona geografica.
 
Il paradigma del \emph{carbon-aware computing} si fonda su questa
osservazione: anziché trattare ogni kilowattora come equivalente, le
decisioni di esecuzione possono essere modulate in funzione della carbon
intensity corrente, con l'obiettivo di concentrare il consumo nei
momenti o nei luoghi in cui l'energia disponibile è più pulita. In
questo modo, il criterio di ottimizzazione si sposta dal solo consumo
energetico all'impatto ambientale complessivo dell'esecuzione, misurato
in termini di emissioni di CO$_2$ associate.
...
servizi specializzati quali ElectricityMaps~\cite{electricitymaps2024} o
WattTime.
...
Tuttavia, analogamente a quanto osservato per gli approcci di
scheduling energy-aware analizzati nella
Sottosezione~\ref{subsec:energy_aware_scheduling}, la decisione
carbon-aware di GreenCourier riguarda esclusivamente \emph{dove}
eseguire la funzione --- in quale regione geografica --- senza
intervenire su \emph{cosa} viene eseguito. La funzione è trattata come
un'entità atomica con un profilo computazionale invariante: lo scheduler
seleziona la regione con il minor impatto carbonico, ma non può
modulare il consumo energetico dell'invocazione stessa. In altri
termini, il segnale di carbon intensity guida la scelta del contesto di
esecuzione, ma non viene utilizzato per orientare la selezione tra
varianti della funzione con profili energia-accuratezza differenziati.
...
di esecuzione, estendendo il criterio di ottimizzazione dal solo
consumo energetico all'impatto ambientale complessivo.
 
Entrambi gli approcci condividono tuttavia una caratteristica comune con
le famiglie di soluzioni esaminate nelle sottosezioni precedenti: la
carbon intensity viene utilizzata per decidere \emph{dove} o
\emph{quando} eseguire un carico di lavoro, ma la funzione eseguita
rimane invariante rispetto alle condizioni della rete elettrica.