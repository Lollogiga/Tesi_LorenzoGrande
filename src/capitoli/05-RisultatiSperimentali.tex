...
differenza del replay sequenziale usato negli Esperimenti~1 e~2, k6
...
due baseline, aggregata sulle prime tre fasi del workload (regime
moderato, senza saturazione). Le colonne $\Delta T$ indicano la
...
\begin{table}[H]
    ...
    \caption{Variazione percentuale di \emph{adaptive-ci} rispetto alle
             due baseline nelle prime tre fasi del workload, usate come
             regime di carico moderato.}
...
accodata fino a quando una risorsa di esecuzione torna disponibile. La
coda ha capacità finita pari a 1000~richieste: se il tempo medio di
...
\paragraph{Invarianza delle scelte carbon-aware rispetto al carico.}
Nella fase di confronto quantitativo, le relazioni tra energia,
emissioni e qualità replicano il pattern degli Esperimenti~1 e~2.
\emph{Adaptive-ci} si colloca in posizione intermedia tra le due
baseline: consuma più di $\Lambda_0$ ma con qualità superiore, e meno
di $\Lambda_1$ con una degradazione di qualità contenuta. La
distribuzione delle varianti selezionate conferma che lo scheduler
risponde al segnale di carbon intensity, non al numero di richieste
concorrenti: la composizione delle scelte varia tra funzioni in funzione
della struttura delle rispettive frontiere di Pareto, non del tasso di
arrivo. Il tempo di risposta riflette il peso computazionale della
variante selezionata, senza overhead misurabile introdotto dal piano di
controllo.

\paragraph{Throughput effettivo e soglia di saturazione.}
Sotto carico crescente emergono differenze operative significative tra
le policy. $\Lambda_0$ completa il 100\% delle richieste in tutte le
fasi, con latenza nell'ordine del millisecondo: le varianti leggere che
seleziona hanno tempi di esecuzione così contenuti da non saturare mai
il nodo. La policy \emph{adaptive-ci} completa il 99\% delle richieste
sull'intero workload; i pochi fallimenti si concentrano nella fase
\emph{Peak} (25~req/s) e corrispondono a timeout dovuti a code
transitorie, non a errori di configurazione (HTTP~422 è sempre zero
per tutte le policy).

$\Lambda_1$ mostra un comportamento qualitativamente diverso: già dalla
fase \emph{Medium} (5~req/s) il throughput effettivo diverge dal tasso
configurato e si stabilizza intorno a 1,25~req/s, indipendentemente da
quanto il carico aumenti nelle fasi successive. Il nodo non collassa,
ma raggiunge il suo limite di capacità: la scelta sistematica di
varianti computazionalmente costose abbassa la capacità di servizio
effettiva del sistema fino a un valore fisso, determinato dal tempo
medio di esecuzione delle varianti ad alta qualità.

Questo risultato ha rilievo autonomo: mostra che il costo operativo di
una strategia di scheduling non può essere valutato indipendentemente
dal profilo di carico. Una policy che privilegia sempre la qualità può
essere sostenibile a basso traffico, ma introduce un tetto fisso al
throughput raggiungibile a carichi medi o elevati. Lo scheduler
carbon-aware, selezionando varianti più leggere nelle fasi di alta CI,
mantiene una capacità di servizio sufficiente a seguire il profilo di
arrivo in tutti i regimi testati.
