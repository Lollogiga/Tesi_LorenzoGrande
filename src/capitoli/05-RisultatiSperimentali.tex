...
Tabella~\ref{tab:baseline-comparison-exp1} riporta, per energia e qualità,
la variazione percentuale dello scheduler rispetto a entrambe le baseline.
L'energia media è ricostruita dalle distribuzioni di varianti osservate e
dai profili energetici Pareto, mentre la qualità media utilizza i
\texttt{quality\_stats} registrati nei summary sperimentali. Un valore negativo di
$\Delta E$ indica che lo scheduler consuma meno della baseline, mentre un
...
\begin{table}[H]
    ...
    \caption{Variazione percentuale dello scheduler carbon-aware rispetto
             alle due baseline, ricalcolata usando le energie statiche
             definite nei file JSON delle varianti, la distribuzione
             delle varianti nei summary originali e il tempo medio
             imputato per variante. $\Delta E_{acc}$, $\Delta T_{acc}$,
             ...
             indicano risparmio rispetto alla baseline; valori positivi
             di $\Delta Q$ indicano qualita superiore alla baseline.}
    \label{tab:baseline-comparison}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{llrrrr}
        \toprule
        Funzione & Zona
            & $\Delta E_{acc}$ & $\Delta Q_{acc}$
            & $\Delta E_{ene}$ & $\Delta Q_{ene}$ \\
        \midrule
        % ── PiLeibniz ──────────────────────────────────────────────────────────
...
Le differenze tra le funzioni derivano dalla forma della frontiera di
Pareto: quando la frontiera è densa, come per alcune funzioni numeriche,
il passaggio tra varianti è graduale; quando è composta da pochi modelli
distanti, come nelle funzioni ML, i salti di qualità sono più marcati,
coerentemente con quanto osservato nella
